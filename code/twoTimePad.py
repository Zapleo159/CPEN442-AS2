...
def testMessageText(file_name, test_word, ciphertexts_xor):
  with open(f"./output_text_files/{file_name}.txt", "w") as f:
    ...
    for i in range(0, len(ciphertexts_xor) - len(test_word_binary), 7):
      ...
      # print(f"{i}: {binary_to_string(relevant_xor)}")
...
def group6(file_name, test_word_formatted):
  ...
  test_word = 'Crop diversity loss threatens global food security, as the world\'s human population depends on a diminishing number of varieties of a diminishing number of crop species. Crops are increasingly grown in monoculture, meaning that if, as in the historic Great Famine of Ireland, a single disease overco'
  # testMessageText(test_word, test_word, ciphertexts_xor)
  testMessageText("full_text", test_word, ciphertexts_xor)
  # testMessageText(file_name, test_word_formatted, ciphertexts_xor)

def group20():
  plaintexts = readfile('./assignment2_files/q2_plaintexts_group20.txt')
  print(plaintexts, '\n')
  
  ciphertexts = readfile('./assignment2_files/q2_ciphertexts_group20.txt')
  # print(ciphertexts)
  
  keygroup = readfile('./assignment2_files/q2_key_group20.txt')
  # print(keygroup)
